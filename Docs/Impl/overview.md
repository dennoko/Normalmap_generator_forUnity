> **注意：** この文書は Python 実装からの**移植時**の要件定義であり、現行実装とは以下が異なります。
> プレビュー処理・距離変換・カラースペースの現行仕様は [`preview-pipeline.md`](preview-pipeline.md) を参照してください。
>
> - プレビューは 512×512 固定ではなく `min(元解像度, 上限)`（既定 2048）
> - `JFA_Distance` は半径でクランプせず生の距離を出力する（キャッシュのため）
> - JFA の開始ステップは画像サイズではなくベベル半径基準、末尾に JFA+1 パスあり
> - シードはピクセル中心ではなくサブピクセル位置に置く
> - プロファイル適用時のブラーは固定 3×3 ではなく半径連動の分離可能ガウシアン
> - `ProfileType` に `Smoothstep` を追加
> - `BevelRadius` / `Strength` は int ではなく float
> - しきい値・グレー値は表示（sRGB）空間で評価する
